...
for pat in ${samples[@]}; do
  ...
  aws s3 cp s3://melanoma-ribas/cellranger/v7.0.0/${pat}/filtered_feature_bc_matrix/barcodes.tsv.gz data/${pat}/ --quiet

  single_cell_tools/numbat/pileup_and_phase.R --label ${pat} \
  --samples ${pat} \
  --bams data/${pat}/possorted_genome_bam.bam \
  --barcodes data/${pat}/barcodes.tsv.gz \
